Scrapes https://www.pricecharting.com for price data on the top 50 most expensive products in a set, for all set names stored in a S3 `config.json` file, then writes as CSV to another S3 bucket (price-charting-scraper-output) to trigger downstream Lambda (merge-sources)
